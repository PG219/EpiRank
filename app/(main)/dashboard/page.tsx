...
export default function HomePage() {
  ...
  const [filters, setFilters] = useState({ region: 'all', status: 'all', limit: 50 })
  ...
  // Compute novel vs known from real rankings
  const knownCount = rankings?.filter(g => g.status === 'known').length ?? 0
  const novelCount = rankings?.filter(g => g.status === 'novel').length ?? 0
...
const statsCards = [
  ...
  if (rankings && rankings.length > 0) {
    console.log("Sample gene:", rankings[0])
    console.log("Known count:", rankings.filter(g => g.status === 'known').length)
  }
  }, [rankings])
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex justify-between items-center">
        <div>
          ...
          <motion.p variants={itemVariants} className="text-slate-400 mt-2 flex items-center gap-2">
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-4 gap-4">
        {statsCards.map((card, index) => (
          ...
          >
            ...
            <div className={`absolute -inset-0.5 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 blur transition-all duration-700 ${card.borderGlow}`} />
            <div className="relative z-10">
              ...
              <p className="text-sm text-slate-400 group-hover:text-slate-300 mt-1">{card.title}</p>
            </div>
        ...
        <div className="relative bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
          <div className="flex gap-4 items-start">
            <div className="flex-1">
              <SearchBar value={searchQuery} onChange={setSearchQuery} placeholder="Search by gene name..." />
            </div>
            <motion.button
          ...
          <div className="mt-4 flex gap-2 flex-wrap">
            {filters.region !== 'all' && (
              <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full text-xs text-blue-300 flex items-center gap-1">
                Region: {filters.region}
                <button className="hover:text-white transition-colors">×</button>
              </motion.span>
            )}
            {filters.status !== 'all' && (
              <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="px-3 py-1 bg-purple-500/20 border border-purple-500/30 rounded-full text-xs text-purple-300 flex items-center gap-1">
                Status: {filters.status}
                <button className="hover:text-white transition-colors">×</button>
              </motion.span>
            )}
            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="px-3 py-1 bg-slate-500/20 border border-slate-500/30 rounded-full text-xs text-slate-300">
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-8">
          <div className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500/50 to-purple-500/50 rounded-2xl opacity-0 group-hover:opacity-40 blur transition duration-1000" />
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
              ...
              <RankTable data={rankings || []} isLoading={isLoading} onGeneClick={handleGeneClick} />
            </div>
          </div>
        </div>
            ...
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5">
              ...
              <div className="space-y-3">
                {['Methylation Delta', 'Histone H3K4me3', 'Network Degree', 'Chromatin Accessibility'].map((feature, i) => (
                  <motion.div key={feature} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.1 }} className="flex items-center justify-between group/feature">
                    <span className="text-sm text-slate-400 group-hover/feature:text-white transition-colors">{feature}</span>
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                        <motion.div className="h-full bg-gradient-to-r from-blue-400 to-purple-400" initial={{ width: 0 }} animate={{ width: `${85 - i * 15}%` }} transition={{ duration: 1, delay: i * 0.2 }} />
                      </div>
                      <span className="text-xs font-mono text-slate-300">{(0.85 - i * 0.15).toFixed(2)}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {(rankings ?? []).slice(0, 3).map((gene) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-white">{gene.gene}</span>
                      <StatusBadge status={gene.status} />
                    </div>
                    <span className="text-xs text-slate-200">#{gene.rank}</span>
                  </motion.div>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="text-center p-2 rounded-lg bg-white/5">
                  <div className="text-lg font-bold text-white">
                    {metrics ? metrics.precision_at_20 : '...'}
                  </div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                      Rank #{selectedGene.rank} • Score {selectedGene.score.toFixed(3)}
                    </p>
                  </div>
                  <motion.button
